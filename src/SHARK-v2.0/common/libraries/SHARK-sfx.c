  ...
  switch (command)
  {
    case sfx_play:
         ...
         if (i == (g_sfx -> mix_count - 1))
         {
           ...
           i = 0;
           while ((i < (g_sfx -> mix_count - 1)) && (g_sfx -> mix_samples[i] != NULL)) i++;
           if (i < (g_sfx -> mix_count - 1))
           {
...
_SfxStreamCallback(snd_stream stream_id, void *user, void *buffer, uint32 frames)
{
  ...
  else
  {
    ...
    if (g_sfx -> format == snd_uint8)
    {
      while (cnt--) *p++ = 0x80;  // unsigned, zero is 0x80
    }
    else
    {
      while (cnt--) *p++ = 0;
  ...
  {
    s = g_sfx -> mix_samples[i];
    if ((s != NULL) && (!s -> active)) g_sfx -> mix_samples[i] = NULL;
  }
