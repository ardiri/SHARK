...
typedef struct _GfxGlobals
{
  ...
  gfx_window *(*fnGfxCreateWindow)(coord, coord, boolean);
  gfx_window *(*fnGfxCreateWindowFromBuffer)(coord, coord, void *, boolean);
  gfx_window *(*fnGfxCreateWindowFromBitmapStream)(coord, coord, lcd_format *, void *, boolean);
...
gfx_window *
_GfxCreateWindow(coord width, coord height, boolean storage)
{
  gfx_window *result;
  GFX_GLOBALS_ACCESS;
  ...
  if (g_gfx -> fnGfxCreateWindow != NULL)
    result = g_gfx -> fnGfxCreateWindow(width, height, storage);

  return result;
}

gfx_window *
_GfxCreateWindowFromBuffer(coord width, coord height, void *bits, boolean storage)
{
  gfx_window *result;
  GFX_GLOBALS_ACCESS;
  ...
  // entry requirement (must call _GfxInitialize())
  if ((g_gfx == NULL) || (width == 0) || (height == 0) || (bits == 0)) return result;

  // execute the sub-graphics layer command
  if (g_gfx -> fnGfxCreateWindowFromBuffer != NULL)
    result = g_gfx -> fnGfxCreateWindowFromBuffer(width, height, bits, storage);

  return result;
}

gfx_window *
_GfxCreateWindowFromBitmapStream(coord width, coord height, lcd_format *format, void *bits, boolean storage)
{
  gfx_window *result;
  GFX_GLOBALS_ACCESS;
  ...
  if (g_gfx -> fnGfxCreateWindowFromBitmapStream != NULL)
    result = g_gfx -> fnGfxCreateWindowFromBitmapStream(width, height, format, bits, storage);

  return result;
}

gfx_window *
_GfxCreateWindowFromFile(uint32 disk, char *filename, rectangle *bounds, boolean storage)
{
  gfx_window *result;
  coord       width, height;
...
_GfxInitialize_RESIZE()
{
  ...
  g_gfx -> LCD = _GfxCreateWindow(g_gfx -> width, g_gfx -> height, false);
