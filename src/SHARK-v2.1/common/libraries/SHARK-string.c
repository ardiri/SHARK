...
{
  _MemMove((void *)s, (void *)sX, (uint32)(_StrLen(sX) + 1));
}
...
{
  _MemMove((void *)s, (void *)sX, (uint32)(MIN(count, _StrLen(sX)) + 1));
}

void
_StrNCat(char *s, char *sX, uint32 count)
{
  _MemMove((void *)(s + _StrLen(s)), 
           (void *)sX, (uint32)(MIN(count, _StrLen(sX)) + 1));
}
