...
typedef enum { iterator_start = 0, iterator_active, iterator_stop = 255 } file_iterator;
...
extern boolean     _DiskEnumerate(file_iterator *iterator, uint32 *disk, char *name);
extern boolean     _FileEnumerate(uint32 disk, file *dir, file_iterator *iterator, char *name, uint32 *attributes);
...
extern uint16      _FileWrite(file *f, void *buf, uint32 objSize, uint32 numObject);
...
#define INPUT_STYLUS            0x00000001
#define INPUT_KEY               0x00000002
#define INPUT_KEY_ANALOG        0x00000004
...
extern gfx_window *_GfxCreateWindowFromBuffer(coord width, coord height, void *bits);
extern gfx_window *_GfxCreateWindowFromBufferDisposable(coord width, coord height, void *bits);
extern gfx_window *_GfxCreateWindowFromBitmapStream(coord width, coord height, lcd_format *format, void *bits);
extern gfx_window *_GfxCreateWindowFromFile(uint32 disk, char *filename, rectangle *bounds);
...
extern void        _MnuReset();
extern uint32      _MnuGetHeight();
extern void        _MnuSetFont(font font_id);
extern void        _MnuSetButton(int8 pos, char *label, uint8 key_code);
extern void        _MnuSetMenuTriggerRegion(rectangle *rect);
extern void        _MnuAddItem(char *label, uint8 key_code, char *param);
extern void        _MnuAddSeparator();
extern void        _MnuComplete();
...
#define GUI_NORTH      GUI_TOP
#define GUI_SOUTH      GUI_BOTTOM
#define GUI_EAST       GUI_LEFT
#define GUI_WEST       GUI_RIGHT

#define FLOW_LAYOUT(layout, hg, vg, o, j)           \
  (layout).type = gui_flow;                         \
  (layout).hgap = (hg);                             \
  (layout).vgap = (vg);                             \
  (layout).data.flowlayout.orientation   = (o);     \
  (layout).data.flowlayout.justification = (j);  

#define GRID_LAYOUT(layout, hg, vg, c, r)           \
  (layout).type = gui_grid;                         \
  (layout).hgap = (hg);                             \
  (layout).vgap = (vg);                             \
  (layout).data.gridlayout.cols = (c);              \
  (layout).data.gridlayout.rows = (r);  

#define GRID_LAYOUT_POSITION(pos, c, r)             \
  (pos).data.gridlayout.col = (c);                  \
  (pos).data.gridlayout.row = (r);                

#define BORDER_LAYOUT(layout, hg, vg)               \
  (layout).type = gui_border;                       \
  (layout).hgap = (hg);                             \
  (layout).vgap = (vg);

#define BORDER_LAYOUT_POSITION(pos, z)              \
  (pos).data.borderlayout.zone = (z);

#define STRETCH_LAYOUT(layout, o, w, h)             \
  (layout).type = gui_stretch;                      \
  (layout).data.stretchlayout.orientation = (o);    \
  (layout).data.stretchlayout.width       = (w);    \
  (layout).data.stretchlayout.height      = (h); 

#define STRETCH_LAYOUT_POSITION(pos, _x, _y, w, h)  \
  (pos).data.stretchlayout.x      = (_x);           \
  (pos).data.stretchlayout.y      = (_y);           \
  (pos).data.stretchlayout.width  = (w);            \
  (pos).data.stretchlayout.height = (h);
...
{
  gfx_window   *image;
  gfx_window   *image_inactive;
  boolean       pressed;
} gui_imagebutton_info;
...
gui_element *_GUINew(gui_type type, void *info);
void         _GUIFree(gui_element *e);

void         _GUISetPosition(gui_element *e, coord x, coord y);
void         _GUISetSize(gui_element *e, int16 width, int16 height);
void         _GUISetLayout(gui_element *e, gui_layout *layout);
void         _GUIGetMinimumSize(gui_element *e, int16 *width, int16 *height);

boolean      _GUIInsert(gui_element *e, gui_element *item);
boolean      _GUIInsertAtPosition(gui_element *e, gui_element *item, gui_position *pos);
void         _GUIRemove(gui_element *e, gui_element *item);
